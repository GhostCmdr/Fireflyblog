...
function measureScrollbarWidth() {
	...
	d.style.cssText =
		"overflow:scroll;position:absolute;top:-9999px;width:100px;height:100px;";
...
function freezeWallpaperForEditor(scrollbarWidth: number) {
	...
	var img =
		(ww.querySelector(
			".slide-item.active.hidden.lg\\:block img",
		) as HTMLImageElement) ||
	...
	function apply(iw: number, ih: number) {
		...
		var opRaw = (getComputedStyle(img).objectPosition || "50% 50%")
			.trim()
			.split(/\s+/);
		// biome(noParameterAssign)：不给参数重新赋值 —— 改用局部常量，语义完全一致
		function parsePos(raw: string) {
			const p = raw.toLowerCase();
			if (p === "center") return { v: 0.5, pct: true };
			if (p === "left" || p === "top") return { v: 0, pct: true };
			if (p === "right" || p === "bottom") return { v: 1, pct: true };
			if (p.indexOf("%") >= 0)
				return { v: Number.parseFloat(p) / 100, pct: true };
			return { v: Number.parseFloat(p) || 0, pct: false }; // px / 数值按绝对偏移
		}
...
function applyEditorModeLayout() {
	...
	var navbarCss =
		"body.editor-page.sticky-navbar #top-row, body.editor-page.dynamic-navbar #top-row { left: 0px !important; right: " +
		rightInset +
		"px !important; transition: none !important; }";
	...
	if (
		document.documentElement.getAttribute("data-wallpaper-mode") === "overlay"
	) {
...
function syncEditorBgVideo() {
	...
	if (
		...
		if (playing) {
			...
			if (!layer2) {
				// 重新测量滚动条宽度并恢复冻结壁纸
				// biome(noInnerDeclarations)：同上，var → const（仅本块使用）
				const _d = document.createElement("div");
				_d.style.cssText =
					"overflow:scroll;position:absolute;top:-9999px;width:100px;height:100px;";
				document.body.appendChild(_d);
				const _sbw = _d.offsetWidth - _d.clientWidth;
...
function handleVisitStart(visit: { to: { url: string } }): void {
	...
	const _isGoingToEditor =
		_visitPath === "/editor/" || _visitPath === "/editor";
	const _isCurrentlyEditor =
		location.pathname === "/editor/" || location.pathname === "/editor";
	if (_isGoingToEditor && !_isCurrentlyEditor) {
		// 进入编辑器：只保存当前模式，不切换（避免主页动画变透明）
		const curMode = document.documentElement.getAttribute(
			"data-wallpaper-mode",
		);
